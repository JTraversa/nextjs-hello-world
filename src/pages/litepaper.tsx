...
The rate of growth of Goo happens to be maximized when $Goo = Mult$. Thus, there is a clear comparison with Uniswap V2's $x * y = k$ constant product market maker design, where the point of maximization is also the point at which $X = Y$.
...
Since $F$ represents a fixed fraction of the pool. As the pool grows, so to will the assets redeemable by the fraction.

On withdraw, a user exchanges some $F$ for $Goo$ and/or $Mult$, burning the respective amount of $F$ in the process.
...
We can derive the amount of reserves allotted to an amount of $F_{d}$ from the inverse of the issuance calculation:
...
subsituting, [rearranging](https://www.wolframalpha.com/input?key=&i=1%2F4%28%28M%2BA%29t%29%5E2+%2B+t+*+sqrt%28%28M+%2B+A%29%28G+-%28G%2F%281+%2B+%28M%2FA%29%29%29%29%29+%3D+1%2F4%28M*t%29%5E2+%2B+X+%2B+t+*+sqrt%28M%28G-%28G%2F%281%2B%28M%2FA%29%29%29%2BX%29%29%2C+find+X) and simplifying, we get

$$
Goo_x = {t^2\\over2}({Mult_iMult_{avg}+Mult_i+{{2\\sqrt{Goo_iMult_i}\\over t}} + {{Mult_{avg}}^2\\over 2}}) + Goo_{x_2}
$$

where
$$
Goo_{x_2} = {Mult_i\\over t}{{\\sqrt{{{4Goo_i\\over Mult_{avg} + Mult_i} + 2Mult_{avg} + {t}^2 + {4t\\sqrt{Goo_iMult_i} + Mult_{avg}^2 {t}^2\\over Mult_i}} }}}
$$

We can then subtract the discount $Goo_x$ from the known revealed Gobbler price $P_r$ to arrive at the price we can pay for an unrevealed Gobbler
...
You can reach us on Twitter at [@mevbandit](https://twitter.com/mevbandit), [@0xAlcibiades](https://twitter.com/0xalcibiades), and [@CapitalGrug](https://twitter.com/CapitalGrug).
`;
const litepaperAuthors = [
	{ name: 'Bandit', link: 'https://twitter.com/mevbandit' },
...
const litepaperAck: [name: string, link: string][] = [
	...
	['perpetuum7', 'https://twitter.com/_perpetuum7'],
	['horsefacts', 'https://twitter.com/eth_call'],
	['neodaoist', 'https://twitter.com/neodaoist'],
	['Marcus Wentz', 'https://github.com/MarcusWentz'],
	['Achal Srinivasan', 'https://www.paradigm.xyz/team/achalsrinivasan'],
	['pluto', 'https://twitter.com/homeslashpluto'],
	['Ben Leimberger', 'https://twitter.com/ben_leimberger']
];
...
const LitepaperPage: ExtendedNextPage<LitepaperPageProps> = ({ content }) => {
	return (
		<>
			<div className="w-screen bg-white pt-[4.6rem] text-black">
				<div className="litepaper__container">
					<div className="mx-auto max-w-full sm:max-w-2xl">
						<h1 className="mb-5 font-paradigm text-[2.75rem] font-normal leading-[1.3]">Goober Litepaper</h1>
						...
						<p className="mb-6 font-authors">
							...
							<p>
								<ol className="list-inside list-decimal text-xs">
									...
									<li>
										<ScrollingToCLink id="solution" text="Solution" />
									</li>
									<li>
										<ScrollingToCLink id="mechanism" text="Mechanism" />
										<ol className="ml-6 list-inside list-decimal text-xs">
											<li>
												<ScrollingToCLink id="overview" text="Overview" />
											</li>
											<li>
												<ScrollingToCLink id="optimizing-goo-production" text="Optimizing Goo production" />
											</li>
											<li>
												<ScrollingToCLink id="gbr" text="GBR" />
												<ol className="ml-6 list-inside list-decimal text-xs">
													<li>
														<ScrollingToCLink id="protocol-fees" text="Protocol Fees" />
													</li>
												</ol>
											</li>
											<li>
												<ScrollingToCLink id="issuance" text="Issuance" />
											</li>
											<li>
												<ScrollingToCLink id="redemption" text="Redemption" />
											</li>
											<li>
												<ScrollingToCLink
													id="liquid-markets-for-goo-and-gobblers"
													text="Liquid markets for Goo and Gobblers"
												/>
												<ol className="ml-6 list-inside list-decimal text-xs">
													<li>
														<ScrollingToCLink
															id="balancing-the-swap-with-erroneous-goo"
															text="Balancing the swap with erroneous Goo"
														/>
													</li>
												</ol>
											</li>
											<li>
												<ScrollingToCLink id="price-impact" text="Price impact" />
											</li>
											<li>
												<ScrollingToCLink id="flash-swaps" text="Flash swaps" />
											</li>
											<li>
												<ScrollingToCLink id="minting" text="Minting" />
												<ol className="ml-6 list-inside list-decimal text-xs">
													<li>
														<ScrollingToCLink id="definitions" text="Definitions" />
													</li>
													<li>
														<ScrollingToCLink
															id="discounting-unrevealed-gobblers"
															text="Discounting unrevealed Gobblers"
														/>
													</li>
												</ol>
											</li>
									...
									<li>
										<ScrollingToCLink id="code" text="Code" />
									</li>
									<li>
					...
					<div className="pb-8" />
					<hr />
					<div className="pb-32 pt-2 font-authors text-[12px] text-black">
						Acknowledgments:{' '}
